...
line3x = linspace(B(1),X(nbpoints,1),evaliter);
line3y = linspace(B(2),X(nbpoints,2),evaliter);
...
for circ = 1:nbcircles
    for iter = 1:evaliter
        ...
        g1((circ-1)*((nbpoints+1)*evaliter) + (nbpoints*evaliter)+iter) = circle(circ,3)*(1+tolerance) - sqrt((line3x(iter) - circle(circ,1))^2 + (line3y(iter) - circle(circ,2))^2);
    end
